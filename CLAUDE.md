...
| Phantom entities the generator INFERRED rather than read from the SRS | `bundle_integrity_check` → AUTOCREATED_ENTITY (**warning**, never blocks export). Every other check asks "does this UUID resolve?" — a phantom answers yes, because the mappings pointing at it are well-formed. This one asks "should this entity exist?", which only the SRS can answer, so it flags `_autoCreated: true` for human/agent verification. Seen twice on real SRSes (a qualified target subject `"<Subject> (female)"`, and program names leaking into a subject column), each time owning an entire program's mappings while validator + FK checks reported zero errors. Warning, not error, because auto-creation is sometimes legitimate. |
