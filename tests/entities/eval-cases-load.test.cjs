...
// under `npm test`: all 23 case files parse + expose their contract, the fixture
...
test("eval registry: exactly 23 case files, numbered 01..23", () => {
  const files = caseFiles();
  assert.equal(files.length, 23, `expected 23 case files, got ${files.length}: ${files.join(", ")}`);
  const nums = files.map((f) => f.slice(0, 2));
  const expected = Array.from({ length: 23 }, (_, i) => String(i + 1).padStart(2, "0"));
...
test("eval cases: all 23 are implemented (no pending stubs remain)", () => {
