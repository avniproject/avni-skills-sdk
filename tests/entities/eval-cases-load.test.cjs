...
// under `npm test`: all 29 case files parse + expose their contract, the fixture
...
test("eval registry: exactly 29 case files, numbered 01..29", () => {
  const files = caseFiles();
  assert.equal(files.length, 29, `expected 29 case files, got ${files.length}: ${files.join(", ")}`);
...
test("eval cases: all 29 are implemented (no pending stubs remain)", () => {
